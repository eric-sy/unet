...
def adjustData(img, mask, flag_multi_class, num_class):
    if (flag_multi_class):
        ...
        new_mask = np.reshape(new_mask, (
            new_mask.shape[0], new_mask.shape[1] * new_mask.shape[2], new_mask.shape[3])) if flag_multi_class else np.reshape(
...
def saveResult(save_path, npyfile, save_name, flag_multi_class=False, num_class=2):
    for i, item in enumerate(npyfile):
        img = labelVisualize(num_class, COLOR_DICT, item) if flag_multi_class else item[:, :, 0]
        io.imsave(os.path.join(save_path, save_name[i]), img)
